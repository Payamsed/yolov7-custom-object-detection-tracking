        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                if opt.track:
                    ...
                    # cv2.line(im0,(limit[0],limit[1]),(limit[2],limit[3]),(0,0,255),1)
                    for r in tracked_dets:
                      x_1,y_1,x_2,y_2 = r[:4]
                      x_1,y_1,x_2,y_2 = int(x_1),int(y_1),int(x_2),int(y_2)
                      idd = r[-1]
                      w_,h_ = x_2 -x_1, y_2-y_1
                      cv2.rectangle(im0,(x_1,y_1),(x_2,y_2),thickness = 5, color = (255,0,0))
                      
                      c_x,c_y = x_1 + w_/2, y_1+ h_/2
                      c_x,c_y = int(c_x),int(c_y)
                      cv2.circle(im0,(c_x,c_y),3,(255,0,255))

                      if limit[0]<c_x<limit[2] and limit[1]-15<c_y<limit[3]+15:
                        if ct.count(idd) == 0:
                          # cv2.line(im0,(limit[0],limit[1]),(limit[2],limit[3]),(0,255,0),6)
                          ct.append(idd)
                      cv2.putText(im0,f'count:{str(len(ct))}',org = (480,550),fontFace = cv2.FONT_HERSHEY_DUPLEX,fontScale = 1.0,color = (255, 0, 255),thickness = 2)
